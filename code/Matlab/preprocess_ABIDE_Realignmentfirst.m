...
%Place to store slice timing corrected files
% slicePath = "D:\neurodynamics\data\ABIDE\Preprocessed\sliceTimingCorrected";
% if ~isfolder(slicePath)
%     mkdir(slicePath) 
% end
% if ~isfolder(join([slicePath, "\ABIDE_I"], ""))
%     mkdir(join([slicePath, "\ABIDE_I"], ""))
% end
% if ~isfolder(join([slicePath, "\ABIDE_II"], ""))
%     mkdir(join([slicePath, "\ABIDE_II"], ""))
% end
...
%% Trunctation to 146 time points
nrOfTimePoints = 146;
...
for i = 1:length(rest_files)
    if checkFilePath(rest_files(i), 'BPF', out_dir, true)
        ...
        if isfile(truncPath) || isfile(truncPath(1:end-3))
            fprintf("Already truncated: %s\n", truncPath)
        else 
